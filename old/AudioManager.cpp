...
void AudioManager::playSongID(int id) {
	...
	Ease::create(0.f, 1.f, 0.5f, &this->volume);
}
...
void AudioManager::playNewSong() {
	...
	// this should NOT have to be checked here but it breaks sometimes if it isnt
	// literally have zero idea why
	if (this->isWaitingToPlayNewSong) return geode::log::info("isWaitingToPlayNewSong is true! cancel playNewSong");
...
void AudioManager::nextSong() {
	if (this->hasNoSongs) return;
	if (this->isWaitingToPlayNewSong) return geode::log::info("isWaitingToPlayNewSong is true! nextSong is cancelled");

	this->isWaitingToPlayNewSong = true;
	Ease::create(1.f, 0.f, .5f, &this->volume, [this]() {
		this->isWaitingToPlayNewSong = false;
		this->playNewSong();
	});
}
...
void AudioManager::prevSong() {
	...
	if (this->isWaitingToPlayNewSong) return geode::log::info("isWaitingToPlayNewSong is true! prevSong is cancelled");;
...
void AudioManager::updateLowPassFilter() {
	...
	if (this->lowPassStrength > 1) {
		...
		Ease::create(this->actualLowPassCutoff, 1650.f - this->lowPassStrength * 250, .5f, &this->actualLowPassCutoff);
	} else {
